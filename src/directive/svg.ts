...
export function svgWheel(app: any) {
...
export function svgDrag(app: any) {
      ...
      if (debounce) {
        ...
        if (isStartMoveSvg) {
          ...
          if (clientX !== 0 && clientY !== 0) {
            // 乘以3用于加快拖动的速度
            arrPoint[0] = String(Number(arrPoint[0]) - (e.clientX - clientX) * 3)
            arrPoint[1] = String(Number(arrPoint[1]) - (e.clientY - clientY) * 3)
